...
// Helper function to standard Base64 encode strings (for SSN and Password)
...
export default function EditStaffPage() {
  ...
  // Helper to format API dates (2026-06-10T00:00:00) into Input dates (2026-06-10)
  ...
  useEffect(() => {
    ...
    getStaffById(Number(id))
      .then(res => {
        ...
        // If the API returns the SSN already in Base64, we decode it to show raw text in the form
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      // Re-append the T00:00:00 time format your API expects
      ...
      await updateStaff({
        ...
        firstName: firstName,
        lastName: lastName,
        ssn: toBase64(ssn), // Base64 encoding required by backend
        email: email,
        phone: phone,
        passwordHash: toBase64(password), // Base64 encoding required by backend
        role: role,
        address: address,
